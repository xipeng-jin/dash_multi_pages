...
class MultiPagesBaseModel(BaseModel):
    ...
    def add_type(cls, field_name: str, new_type: Type[Any]):
        ...
        def _is_discriminated_union(field):
            ...
            return (
                hasattr(field.outer_type_, "__metadata__")
                and get_args(field.outer_type_)[1].discriminator
            )
        ...
        # dash_multi_pages.models. The vm.__dict__.copy() is inspired by pydantic's own implementation of update_forward_refs and
        # effectively replaces all ForwardRefs defined in dash_multi_pages.models.
        import dash_multi_pages.models as dmm

        cls.update_forward_refs(**dmm.__dict__.copy())
        new_type.update_forward_refs(**dmm.__dict__.copy())
