...
from dash_multi_pages import _themes as themes

# from dash_multi_pages.actions._actions_utils import CallbackTriggerDict, _get_component_actions
from dash_multi_pages.managers import data_manager, model_manager
from dash_multi_pages.managers._model_manager import ModelID
from dash_multi_pages.models import MultiPagesBaseModel  # , Action

# from dash_multi_pages.models._action._actions_chain import _action_validator_factory
from dash_multi_pages.models._components._components_utils import (
    _callable_mode_validator_factory,
    _process_callable_data_frame,
)
from dash_multi_pages.models._models_utils import _log_call
from dash_multi_pages.models.types import CapturedCallable

logger = logging.getLogger(__name__)


class Graph(MultiPagesBaseModel):
    """Wrapper for `dcc.Graph` to visualize charts in dashboard.

    Args:
        type (Literal["graph"]): Defaults to `"graph"`.
        figure (CapturedCallable): See [`CapturedCallable`][dash_multi_pages.models.types.CapturedCallable].
        actions (List[Action]): See [`Action`][dash_multi_pages.models.Action]. Defaults to `[]`.
    ...
    figure: go.Figure = Field(
        ...,
        description="Plotly figure to be displayed.",
    )
    ...
    # _validate_callable = validator("figure", allow_reuse=True)(
    #     _process_callable_data_frame
    # )
    ...
    def __call__(self, **kwargs):
        ...
        try:
            ...
            if "theme_selector" in ctx.args_grouping.get("external", {}):
                fig = self._update_theme(
                    fig, ctx.args_grouping["external"]["theme_selector"]["value"]
                )
    ...
    def _update_theme(fig: go.Figure, theme_selector: bool):
        # Basically the same as doing fig.update_layout(template="dash_multi_pages_light/dark") but works for both the call in
